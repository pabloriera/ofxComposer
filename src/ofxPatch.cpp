...
void main (void){\n\
    ...
    title->addButton('s', &bRender, TOGGLE_BUTTON);
...
void ofxPatch::removeListeners(){

    ofRemoveListener(ofEvents().mousePressed, this, &ofxPatch::_mousePressed);
	ofRemoveListener(ofEvents().mouseDragged, this, &ofxPatch::_mouseDragged);
	ofRemoveListener(ofEvents().mouseReleased, this, &ofxPatch::_mouseReleased);
	ofRemoveListener(ofEvents().keyPressed, this, &ofxPatch::_keyPressed);

}


ofxPatch::~ofxPatch(){
    cout << "me muero " << nId << endl;
    if ( image != NULL )
        delete image;
...
//------------------------------------------------------------------- SET's & GET's
bool ofxPatch::setFrag( string _code){
    bool loaded;
    if ( shader != NULL ){
        loaded=shader->setFragmentShader( _code );
        if (loaded){
            saveSettings();
...
void ofxPatch::toggleFullScreen()
{
    bFullScreen=!bFullScreen;
    if(bFullScreen)
    {
        doCornersToPosSize();
        setCoorners(0,0,ofGetWidth(),ofGetHeight());
    }
    else
        setCoorners(x0,y0,width0,height0);
}

void ofxPatch::setFullScreen(bool _bFullScreen)
{
...
void ofxPatch::setCoorners(float x, float y, float w, float h){

    textureCorners[0].set( x,y);
    textureCorners[1].set( x+w,y);
    textureCorners[2].set( x+w,y+h);
    textureCorners[3].set( x,y+h);
...
//
        ...
        ofSetColor(255);
        getTextureReference().draw(0.0,0.0);
        ofPopMatrix();
...
void ofxPatch::doCornersToPosSize()
{
    x0 = textureCorners[0].x;
    y0 = textureCorners[0].y;

    width0 = textureCorners[1].x-textureCorners[0].x;
    height0 = textureCorners[3].y-textureCorners[0].y;
}
    ...
    if (bEditMode){
        if (!bEditMask){
            ...
            if (( selectedTextureCorner >= 0) && ( selectedTextureCorner < 4) ){
                ...
                //
            } else if ( isOver(mouse) && bActive && e.button == 0){
                for (int i = 0; i < 4; i++){
...
bool ofxPatch::reloadFile(bool parameters){

    bool loaded;
    if(type=="ofShader")
    {
        ofBuffer content = ofBufferFromFile( filePath );
        loaded  = setFrag(content.getText());
        if(parameters)
            doParameters();
...
//    }
    ...
    if ((ext == "jpg") || (ext == "JPG") ||
        ...
        if(loaded){
            ...
            title->setTitle(ofToString(nId) + ":" +  name );
            doParameters();
        }
...
bool ofxPatch::loadType(string _type, string _configFile){
    ...
    } else if ( _type == "ofKinect"){
        type = _type;
        kinect = new ofxKinect();

        title->setTitle(ofToString(nId) + ":" + type );
        loaded = kinect->init(true,true,true);
    ...
    if (XML.loadFile(configFile)){
        ...
        if (XML.pushTag("surface", _nTag)){
            ...
            if ( type == "ofVideoGrabber" ){
                ...
                string path = XML.getValue("path", "Error: data not found...");
                ofBuffer buffer = ofBufferFromFile(path);
                loaded  = shader->setFragmentShader(buffer.getText());
